...
return function(s)
	...
	local function create_kb_layout_item(keymap)
		...
		awesome.connect_signal("update::background:kblayout", function()
			awful.spawn.easy_async_with_shell([[ setxkbmap -query | grep layout | awk '{print $2}' ]], function(stdout)
				local layout = stdout:gsub("\n", "")
	...
	kb_menu_widget:connect_signal("mouse::leave", function()
	...
	kb_menu_widget:connect_signal("mouse::enter", function()
		mousegrabber.stop()
	end)

	kb_menu_widget:setup(get_kblist())
			...
			for j, n in ipairs(user_vars.kblayout) do
				if stdout:match(n) then
					if j == #user_vars.kblayout then
						awful.spawn.easy_async_with_shell("setxkbmap " .. user_vars.kblayout[1], function()
							get_kblayout()
						end)
					else
						awful.spawn.easy_async_with_shell("setxkbmap " .. user_vars.kblayout[j + 1], function()
							get_kblayout()
						end)
					end
				end
	...
	awesome.connect_signal("kblayout::toggle", function()
		toggle_kb_layout()
	end)
	...
	kblayout_widget:connect_signal("button::press", function()
		mousegrabber.stop()
	...
	awesome.connect_signal("kblayout::hide:kbmenu", function()
		kb_menu_widget.visible = false
		kblayout_keygrabber:stop()
	end)

...
return function(s)
	...
	local function create_device(name, node, sink)
		...
		if sink == true then
			...
			device:connect_signal("mouse::enter", mouse_enter)

			device:connect_signal("button::press", button_press)

			device:connect_signal("button::release", button_release)

			device:connect_signal("mouse::leave", mouse_leave)
			--#endregion
			...
			awesome.connect_signal("update::background:vol", function(new_node)
				if node == new_node then
					...
					bg = color["nord15"]
					fg = color["nord0"]
					device.background:set_bg(color["nord15"])
					device.background:set_fg(color["nord0"])
				else
					fg = color["nord15"]
					bg = color["nord2"]
					device.background:set_fg(color["nord15"])
					device.background:set_bg(color["nord2"])
				end
			end)
			...
			awesome.connect_signal("update::background:mic", function(new_node)
				if node == new_node then
					...
					bg = color["nord10"]
					fg = color["nord0"]
					device.background:set_bg(color["nord10"])
					device.background:set_fg(color["nord0"])
				else
					fg = color["nord10"]
					bg = color["nord2"]
					device.background:set_fg(color["nord10"])
					device.background:set_bg(color["nord2"])
				end
			end)
			awful.spawn.easy_async_with_shell([[ pactl get-default-source ]], function(stdout)
				local node_active = stdout:gsub("\n", "")
				if node == node_active then
					bg = color["nord10"]
					fg = color["nord0"]
						...
						{
							{
								{
									resize = false,
									image = gears.color.recolor_image(icondir .. "menu-down.svg", color["nord15"]),
									widget = wibox.widget.imagebox,
									id = "icon",
								},
								id = "center",
						...
						{
							{
								{
									resize = false,
									image = gears.color.recolor_image(icondir .. "menu-down.svg", color["nord9"]),
									widget = wibox.widget.imagebox,
									id = "icon",
								},
								id = "center",
					...
					{
						{
							resize = false,
							widget = wibox.widget.imagebox,
							image = gears.color.recolor_image(icondir .. "volume-high.svg", color["nord15"]),
							id = "icon",
						},
						{
							{
								...
								bar_height = dpi(5),
								bar_color = color["nord1"],
								bar_active_color = color["nord15"],
								handle_color = color["nord15"],
								handle_shape = gears.shape.circle,
								handle_border_color = color["nord15"],
								handle_width = dpi(15),
								maximum = 100,
					...
					{
						{
							resize = false,
							widget = wibox.widget.imagebox,
							image = gears.color.recolor_image(icondir .. "microphone.svg", color["nord10"]),
							id = "icon",
						},
						{
							{
								...
								bar_height = dpi(5),
								bar_color = color["nord1"],
								bar_active_color = color["nord10"],
								handle_color = color["nord10"],
								handle_shape = gears.shape.circle,
								handle_border_color = color["nord10"],
								handle_width = dpi(15),
								maximum = 100,
								forced_height = dpi(26),
								widget = wibox.widget.slider,
	...
	audio_selector_margin:connect_signal("button::press", function()
		...
		if volume_list.visible then
			...
			audio_volume.icon:set_image(gears.color.recolor_image(icondir .. "menu-up.svg", color["nord7"]))
		else
			audio_bg.shape = function(cr, width, height)
				gears.shape.rounded_rect(cr, width, height, 4)
			end
			audio_volume.icon:set_image(gears.color.recolor_image(icondir .. "menu-down.svg", color["nord7"]))
		end
	end)
	...
	mic_selector_margin:connect_signal("button::press", function()
		mic_list.visible = not mic_list.visible
		if mic_list.visible then
			mic_selector_margin.mic_bg.shape = function(cr, width, height)
				gears.shape.partially_rounded_rect(cr, width, height, true, true, false, false, 4)
			end
			mic_volume.icon:set_image(gears.color.recolor_image(icondir .. "menu-up.svg", color["nord7"]))
		else
			mic_bg.shape = function(cr, width, height)
				gears.shape.rounded_rect(cr, width, height, 4)
			end
			mic_volume.icon:set_image(gears.color.recolor_image(icondir .. "menu-down.svg", color["nord7"]))
		end
	end)

	local audio_slider_margin =
		volume_controller:get_children_by_id("audio_volume_margin")[1].audio_volume.slider_margin.slider

	-- Volume slider change event
	audio_slider_margin:connect_signal("property::value", function()
		local volume = audio_slider_margin.value
		awful.spawn("pactl set-sink-volume @DEFAULT_SINK@ " .. tonumber(volume) .. "%")
	end)

	local mic_slider_margin =
		volume_controller:get_children_by_id("mic_volume_margin")[1].mic_volume.slider_margin.slider

	-- Microphone slider change event
	mic_slider_margin:connect_signal("property::value", function()
	...
	local volume_controller_container = awful.popup({
		...
		placement = function(c)
			awful.placement.align(c, { position = "top_right", margins = { right = dpi(305), top = dpi(60) } })
		end,
		shape = function(cr, width, height)
			gears.shape.rounded_rect(cr, width, height, 12)
		end,
	...
	local function get_sink_devices()
		awful.spawn.easy_async_with_shell(
	...
	local function get_mic_volume()
		awful.spawn.easy_async_with_shell("./.config/awesome/src/scripts/mic.sh volume", function(stdout)
			local volume = stdout:gsub("%%", ""):gsub("\n", "")
			volume_controller
				:get_children_by_id("mic_volume_margin")[1].mic_volume.slider_margin.slider
				:set_value(tonumber(volume))
			if volume > 0 then
				volume_controller
					:get_children_by_id("mic_volume_margin")[1].icon
					:set_image(gears.color.recolor_image(icondir .. "microphone.svg", color["nord9"]))
			else
				volume_controller
					:get_children_by_id("mic_volume_margin")[1].icon
					:set_image(gears.color.recolor_image(icondir .. "microphone-off.svg", color["nord9"]))
			end
		end)
	end
	...
	local function get_mic_mute()
		awful.spawn.easy_async_with_shell("./.config/awesome/src/scripts/mic.sh mute", function(stdout)
			if stdout:match("yes") then
				volume_controller
					:get_children_by_id("mic_volume_margin")[1].mic_volume.slider_margin.slider
					:set_value(tonumber(0))
				volume_controller
					:get_children_by_id("mic_volume_margin")[1].icon
					:set_image(gears.color.recolor_image(icondir .. "microphone-off.svg", color["nord9"]))
			else
	...
	volume_controller_container:connect_signal("mouse::leave", function()
	...
	volume_controller_container:connect_signal("mouse::enter", function()
		mousegrabber.stop()
	end)
	...
	awesome.connect_signal("get::volume", function(volume)
		...
		volume_controller.controller_margin.controller_layout.audio_volume_margin.audio_volume.slider_margin.slider:set_value(
			volume
		)
		volume_controller.controller_margin.controller_layout.audio_volume_margin.audio_volume.icon:set_image(
			gears.color.recolor_image(icon .. ".svg", color["nord15"])
		)
	end)

	-- Check if the volume is muted
	awesome.connect_signal("get::volume_mute", function(mute)
		if mute then
			volume_controller.controller_margin.controller_layout.audio_volume_margin.audio_volume.icon:set_image(
				gears.color.recolor_image(icondir .. "volume-mute.svg", color["nord15"])
			)
		end
	end)

	-- Set the microphone volume
	awesome.connect_signal("get::mic_volume", function(volume)
		if volume > 0 then
			volume_controller
				:get_children_by_id("mic_volume_margin")[1].mic_volume.icon
				:set_image(gears.color.recolor_image(icondir .. "microphone.svg", color["nord9"]))
		else
			volume_controller
				:get_children_by_id("mic_volume_margin")[1].mic_volume.icon
	...
	awesome.connect_signal("volume_controller::toggle", function(scr)
		if scr == s then

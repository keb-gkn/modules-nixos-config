...
return function(s)
	...
	local update_profile_picture = function()
		awful.spawn.easy_async_with_shell("sh ./.config/awesome/src/scripts/pfp.sh 'userPfp'", function(stdout)
			if stdout then
	...
	local lock_button =
		button("Lock", icondir .. "lock.svg", color["nord10-0"] .. "ff", color["nord10"] .. "ff", lock_command)
	local logout_button =
		button("Logout", icondir .. "logout.svg", color["nord13-0"] .. "ff", color["nord13"] .. "ff", logout_command)
	local suspend_button =
		button("Suspend", icondir .. "suspend.svg", color["nord7-0"] .. "ff", color["nord7"] .. "ff", suspend_command)
	local reboot_button =
		button("Reboot", icondir .. "reboot.svg", color["nord12-0"] .. "ff", color["nord12"] .. "ff", reboot_command)
	local shutdown_button = button(
		"Shutdown",
		icondir .. "shutdown.svg",
		color["nord11-0"] .. "ff",
		color["nord11"] .. "ff",
		shutdown_command
	)

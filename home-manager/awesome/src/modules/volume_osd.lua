...
return function(s)
					...
					{
						{
							...
							bar_active_color = color["nord6"],
							handle_color = color["nord6"],
							handle_shape = gears.shape.circle,
							handle_width = dpi(10),
							handle_border_color = color["nord6"],
							maximum = 100,
	...
	local function update_osd()
		awful.spawn.easy_async_with_shell(
			...
			function(stdout)
				...
				volume_osd_widget.container.osd_layout.icon_slider_layout.label_value_layout.value:set_text(
					volume_level .. "%"
				)
	...
	local update_slider = function()
		awful.spawn.easy_async_with_shell("./.config/awesome/src/scripts/vol.sh mute", function(stdout)

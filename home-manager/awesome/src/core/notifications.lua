...
naughty.connect_signal("request::display", function(n)
	if n.urgency == "critical" then
		n.title = string.format(
			"<span foreground='%s' font='JetBrainsMono Nerd Font, ExtraBold 16'>%s</span>",
			color["RedA200"],
			n.title
		) or ""
										...
										{
											{
												image = gears.color.recolor_image(
													icondir .. "notification-outline.svg",
													color["Teal200"]
												),
												resize = false,
												widget = wibox.widget.imagebox,
	...
	local close =
		w_template.max_size.min_size.widget_layout.arc_app_bg.arc_app_layout.arc_app_layout_2.arc_margin.const.background
...
naughty.connect_signal("destroyed", function() end)
...
naughty.connect_signal("invoked", function(_, action)
